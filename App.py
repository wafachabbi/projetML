...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if run:
        ...
        with cr:
            st.markdown('<div class="sec-head">📊 Similarité <div class="sec-line"></div></div>', unsafe_allow_html=True)
            fig,ax=plt.subplots(figsize=(5,4)); fig.patch.set_facecolor('none')
            ...
            ax.set_title("Score de similarité (%)",fontsize=10,fontweight='bold',color='#0f2347')
            ax.spines[['top','right']].set_visible(False); ax.set_facecolor('#f8faff')
            plt.tight_layout(); st.pyplot(fig)
